...
for QQQ=1:length(folder_list)
   ...
   %% FFT to Frequency Domain
    ...
    if If_Partial_Spectrum ==1
        ...
        Image_Stack_Ori=flip(Image_Stack_Ori,3);
        if If_Hilbert == 0
    ...
    if If_Sharpness_Est == 1
        ...
        NNN=6;
        subplot(1,1,1)
        imagesc(squeeze(abs(Image_Stack(:,NNN,:)))');
        ...
        %% Depth Enhacement
        %Axial_Profile=mean(Reduced_Image,2);
        %Axial_Profile(Axial_Profile>20)=20;
        %plot(1./Axial_Profile)
        BI=-0.1;
        BF=1;
        ...
        colormap(gray);
        %% Basic Index        
        [Gx, Gz]=gradient(Normalized_Image);

        S=sqrt(Gx.^2+Gz.^2)*1000;
        sharpness=mean(mean(S));
        fprintf('\nSharpness Index: %g\n\n',sharpness);
        imwrite(Normalized_Image,[Data_Save_Folder sprintf('%s_Sharpness=%g.png',folder_list(QQQ).name,sharpness)],'png');
        %% Test on Laterally Normalized Image (不能直接normalize gradient, 因為我們不希望noise之gradient被放大; 用max lat norm也會有類似的問題: 有訊號之深度除的值會比沒訊號的深度大很多)
        % 但還是想把intensity normalize掉; 因為相同resolution在不同亮度時,
        % 算出之sharpness盡可能接近
        Latnorm_Image=Reduced_Image./repmat(mean(Reduced_Image,2),[1 size(Reduced_Image,2)]);   %Normalized by mean instead of max
        subplot(1,1,1)
        imagesc(Latnorm_Image)
        colormap(gray);
        caxis([1 5])
        
        [Gx_Latnorm, Gz_Latnorm]=gradient(Latnorm_Image);
        S_Latnorm=sqrt(Gx_Latnorm.^2+Gz_Latnorm.^2)*1000;
        sharpness_Latnorm=mean(mean(S_Latnorm));
        fprintf('\nSharpness Index: %g\n\n',sharpness_Latnorm);
        S_Latnorm_Profile=mean(S_Latnorm,2);
        plot(S_Latnorm_Profile);

        
        
        %% Normalized Gradient

        [Gx_Norm_temp, Gz_Norm_temp]=gradient(Normalized_Image);
        Gx_Norm=Gx_Norm_temp./Normalized_Image;
        Gz_Norm=Gz_Norm_temp./Normalized_Image;
        Gx_Norm(Normalized_Image==0)=0;
        Gz_Norm(Normalized_Image==0)=0;
        
        S_Norm=sqrt(Gx_Norm.^2+Gz_Norm.^2)*1000;    %在此normalized S下, noise之貢獻被高估
        imagesc(abs(S_Norm));
        
        sharpness_Norm=mean(mean((S_Norm.*Normalized_Image)));   %結果應該跟之前的S差不多
