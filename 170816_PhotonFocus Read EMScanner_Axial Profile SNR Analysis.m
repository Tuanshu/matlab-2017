...
root_folder_path='F:\AMO\';
last_folder_name='170816_Forearm Ref_2x';
Number=[];
...
if strcmp(Camera,'MV1')
    Product_of_Axial_Decimation_Factor_and_Ave_Factor=32;%round(2);
elseif strcmp(Camera,'B0620')
    Product_of_Axial_Decimation_Factor_and_Ave_Factor=round(4);
end
ave_factor=32;%Product_of_Axial_Decimation_Factor_and_Ave_Factor;%[4*4*4];
...
for QQQ=1:length(folder_list)
   ...
   %% 
    if If_Axial_Profile == 1
        ...
        plot(Z_Profile,Axial_Profile,'LineWidth',2)
        xlim([0 max(Z_Profile)])
        ylim([0 50])
        xlabel('Depth (micron)')
        ylabel('OCT Signal (a.u.)')
        saveas(gcf,[Data_Save_Folder sprintf('System_%s_AAF_%d_LAF_%d_RBF_%d_CBF_%d_ABTH_%g_AVE_%d_Phase_%d_ProductAVE_%d_Colomn_%d_DeepEnhace_XWidth%d_BI%g_BF%g',System,Axial_ave_Factor,Lateral_Ave_Factor,Row_Binning_Factor,Column_Binning_Factor,Auto_Brightness_TH,ave_factor,Phase_Shift,Product_of_Axial_Decimation_Factor_and_Ave_Factor,Colomn,X_ROI(2)-X_ROI(1),BI,BF) '\' sprintf('%s_Axial_Profile.png',folder_list(QQQ).name)]);
    end
      %%
