...
% %         
% %         % Reshape Method (¥ý¤£°µDownsampling)
          %Image_Virtual_B_Scan=reshape(Image_Stack,size(Image_Stack,1)*size(Image_Stack,2),[]);
%         Usable_Length=floor(size(Image_Virtual_B_Scan,2)/Axial_Window_Size/2)*Axial_Window_Size*2;
%         Image_Virtual_B_Scan=Image_Virtual_B_Scan(:,1:Usable_Length);
%         Image_Parallel_Window=reshape(Image_Virtual_B_Scan,size(Image_Virtual_B_Scan,1),Axial_Window_Size*2,size(Image_Virtual_B_Scan,2)/Axial_Window_Size/2);
%         %
%         imagesc(squeeze(Image_Parallel_Window(:,:,20)));  %1st: virtual B-scan lateral index, 2nd: depth index (in each bin), 3rd: bin index
%         %
%         Image_Auto=ifft(conj(fft(Image_Parallel_Window,[],2)).*fft(Image_Parallel_Window,[],2),[],2);
%         Image_Auto=Image_Auto-repmat(min(Image_Auto,[],2),[1 size(Image_Auto,2) 1]);
%         Image_Ratio=cumsum(Image_Auto(:,1:floor(size(Image_Auto,2)/2),:),2)./repmat(sum(Image_Auto(:,1:floor(size(Image_Auto,2)/2),:),2),[1 floor(size(Image_Auto,2)/2) 1]);       
%         [minvalue0 Image_THindex]=min(abs(Image_Ratio-TH),[],2);
% 
%         Image_THindex_Virtual_Bscan=squeeze(Image_THindex);
%         Speckle_Contrast_Stack=reshape(Image_THindex_Virtual_Bscan,size(Image_Stack,1),size(Image_Stack,2),[]);
%         %
%         %imagesc(squeeze(mean(Speckle_Contrast_Stack(:,:,:),2))');        %
%         imagesc(squeeze(Speckle_Contrast_Stack(:,3,:))'); 
        %
        Image_Virtual_B_Scan=reshape(Image_Stack,size(Image_Stack,1)*size(Image_Stack,2),[]);
        Usable_Length=floor(size(Image_Virtual_B_Scan,2)/Downsampling_Ratio);

        Image_Parallel_Window=zeros(size(Image_Virtual_B_Scan,1),2*Axial_Window_Size+1,Usable_Length);
        for p=1:Usable_Length
            Image_Parallel_Window(:,:,p)=padarray(Image_Virtual_B_Scan(:,max(1,p*Downsampling_Ratio-Axial_Window_Size):min(size(Image_Stack,3),p*Downsampling_Ratio+Axial_Window_Size)),[0 max(0,max(1-(p*Downsampling_Ratio-Axial_Window_Size),p*Downsampling_Ratio+Axial_Window_Size-size(Image_Stack,3)))],'replicate','pre');
            disp(p);
        end
        Image_Auto=ifft(conj(fft(Image_Parallel_Window,[],2)).*fft(Image_Parallel_Window,[],2),[],2);
        Image_Ratio=cumsum(Image_Auto(:,1:floor(size(Image_Auto,2)/2),:),2)./repmat(sum(Image_Auto(:,1:floor(size(Image_Auto,2)/2),:),2),[1 floor(size(Image_Auto,2)/2) 1]);       
...
%
        ...
        Speckle_Image=(Speckle_Image-min(Speckle_Image(:)))./(max(Speckle_Image(:))-min(Speckle_Image(:)));
        imwrite(Speckle_Image,[Data_Save_Folder sprintf('%s_Speckle_Image.png',folder_list(QQQ).name)],'png');

        %%
        %imagesc(squeeze(Speckle_Contrast_Stack(:,10,:))');
