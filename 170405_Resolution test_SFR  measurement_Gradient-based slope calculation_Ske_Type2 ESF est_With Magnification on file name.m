...
for Number=1:TotalNumber
    ...
    %% 17/4/5 ??length(L_Extreme)==length(R_Extreme), ???????edge??(????)
    R_Extrme_First_Index=find(abs(LR_Extreme(1)-R_Extreme)<(500/M),1,'first');
    L_Extrme_Last_Index=find(abs(LR_Extreme-R_Extreme(end))<(500/M),1,'last');
