...
Save_File_Name='170524_EM2 (560 Imperx)_Different Slice Thickness_Y8';
root_folder_path='Z:\研究開發部\何端書\Daily Record\170523_EM2 560 Probe Test\32x_skin\';
last_folder_name=[11];
...
%%        
    ...
    elseif If_EffMap ==0
        if If_Enhace_Deep_Signal
            imwrite(Lateral_Binned_Image,[Processed_Data_Path sprintf('_Bscan_AAF_%d_LAF_%d_RBF_%d_CBF_%d_ABTH_%g_AVE_%d_Phase_%d_ProductAVE_%d_Colomn_%d_DeepEnhace_BI%g_BF%g.png',Axial_ave_Factor,Lateral_Ave_Factor,Row_Binning_Factor,Column_Binning_Factor,Auto_Brightness_TH,ave_factor,Phase_Shift,Product_of_Axial_Decimation_Factor_and_Ave_Factor,Colomn,BI,BF)],'png');
        else
            imwrite(Lateral_Binned_Image,[Processed_Data_Path sprintf('_Bscan_AAF_%d_LAF_%d_RBF_%f_CBF_%d_ABTH_%g_AVE_%d_Phase_%d_ProductAVE_%d_Colomn_%d.png',Axial_ave_Factor,Lateral_Ave_Factor,Row_Binning_Factor,Column_Binning_Factor,Auto_Brightness_TH,ave_factor,Phase_Shift,Product_of_Axial_Decimation_Factor_and_Ave_Factor,Colomn)],'png');
        end
    elseif If_EffMap ==-1
