...
If_Save_Raw=0;
%%
root_folder_path='C:\TuanShu\170724_Pork Test_Change it_CCDZ=7.5mm\';
last_folder_name='Set 1';
Number=[];
...
for QQQ=1:length(folder_list)
   ...
   %% FFT to Frequency Domain
    ...
    if If_Sharpness_Est == 1
        ...
        sharpness_Processed=mean(mean(S_Processed));
   
        %% Normalized Gradient
        Axial_Resolution=0.56;
        Z_Profile=Axial_Resolution*([0:(size(S,1)-1)]);
        ...
        sharpness_Norm=mean(mean((S_Norm)));
        S_Profile=mean(S,2);
        ...
        dlmwrite([Data_Save_Folder 'Sharpness.txt'],[QQQ sharpness sharpness_Processed sharpness_Norm] ,'delimiter','\t','newline','pc','precision', '%.6f','-append');
