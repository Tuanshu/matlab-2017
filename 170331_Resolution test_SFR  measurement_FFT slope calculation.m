...
for Number=1:TotalNumber
    ...
    FFT_Data_Image_Raw=abs(fft2(Blur_Image-mean(Blur_Image(:))));
    FFT_Data_Image_Raw_Shift=[FFT_Data_Image_Raw((floor(size(FFT_Data_Image_Raw,1)/2)+1):end,:); FFT_Data_Image_Raw(1:floor(size(FFT_Data_Image_Raw,1)/2),:)];
    FFT_Data_Image_Raw_Shift=[FFT_Data_Image_Raw_Shift(:,(floor(size(FFT_Data_Image_Raw_Shift,2)/2)+1):end) FFT_Data_Image_Raw_Shift(:,1:floor(size(FFT_Data_Image_Raw_Shift,2)/2))];
    imagesc(FFT_Data_Image_Raw_Shift);
    caxis([0 100000]);
