...
root_folder_path='C:\Users\AMO\Desktop\170609_EMU measurement\Full\cleaned\';
last_folder_name=[3];%
    ...
    if If_Save_3D ==1
        if If_Enhace_Deep_Signal
            ...
            if length(last_folder_name)>1
                Processed_Data_Path=[Processed_Sub_Folder_Name sprintf('%03d_%s.png',QQQ,System)];
            elseif Y_Offset~=0
                Processed_Data_Path=[Processed_Sub_Folder_Name sprintf('%03d_%s_Y%d.png',last_folder_name(QQQ),System,Y_Offset)];
            else
                Processed_Data_Path=[Processed_Sub_Folder_Name sprintf('%03d_%s.png',last_folder_name(QQQ),System)];
            end
        else
