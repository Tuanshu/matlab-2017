...
Wavelength_of_Interest_Array=0.6:0.01:0.9; %micron
...
last_folder_name='170704_New New PZT coef';
Number=[1];
...
for QQQ=1:length(folder_list)
    ...
    if If_STFT_1D
        ...
        for p=1:size(Image_Virtual_B_Scan,1)
            Image_Spectrogram(p,:,:)=spectrogram(Image_Virtual_B_Scan(p,:),Time_Window,0,nBin);  %1st 0 for zero overlapping, nBin for DFT bins, it will outout only half spectrum (nBin+1)
            disp(p);
        end
...
%%
NNN=830;
Temp_Spectrogram=squeeze(Image_Spectrogram(NNN,:,:));
imagesc(abs(Temp_Spectrogram));  %lateral is time
