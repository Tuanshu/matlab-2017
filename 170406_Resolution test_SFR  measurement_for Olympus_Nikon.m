clear all
%%
root_folder_path='D:\170406_20x WIO_100 micron width target\AandP removed\Cleaned\Again Cleaned\Adjed\ADJ 3';
...
for Number=1:TotalNumber
    ...
    [L_Extreme_value L_Extreme]=findpeaks(abs(L_grad),'MINPEAKDISTANCE',round(80/Sampling_Resolution),'MINPEAKHEIGHT',max(abs(L_grad)/3));
    [R_Extreme_value R_Extreme]=findpeaks(abs(R_grad),'MINPEAKDISTANCE',round(80/Sampling_Resolution),'MINPEAKHEIGHT',max(abs(R_grad)/3));
...
%     hold off
    ...
    Upsampling_Ratio=floor(1/Pixel_Phase_Per_Lateral_Pixel)*Further_Oversampling_Ratio;
    ...
    New_Position_Array=Sampling_Resolution*Based_Index_Array_New;   %17/4/5 corrected
...
if If_Best_Only == 1
    ...
    if ESF_Est_Type == 3
        saveas(gcf,[root_folder_path '\' sub_processed_path '\Resolution Chart_' sub_folder_path sprintf('_Min_%g_Fermi.png',min(Resolution_Best))])

    else
        saveas(gcf,[root_folder_path '\' sub_processed_path '\Resolution Chart_' sub_folder_path sprintf('_Min_%g.png',min(Resolution_Best))])
    end
end    
    % %% Find The Best
% Smooth_Window=10;
