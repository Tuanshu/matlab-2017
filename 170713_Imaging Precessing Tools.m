...
%% Post N-point Spatial Frequency Analysis
%% Save Raw File
If_Save_Raw=1;
%%
root_folder_path='C:\TuanShu\';
last_folder_name='170705_Raw Data Examine';
Number=[];
...
for QQQ=1:length(folder_list)
   ...
   %% FFT to Frequency Domain
    ...
    if If_Phase_Map ==1
        ...
        Ave_PostIFFT=1;
        Temp=0;
        Length_Used_PostIFFT=floor(size(Image_Stack_Complex,3)/Ave_PostIFFT)*Ave_PostIFFT;
        for p=1:Ave_PostIFFT
           Temp=Temp+Image_Stack_Complex(:,:,(Ave_PostIFFT-(p-1)):Ave_PostIFFT:(Length_Used_PostIFFT-(p-1)));
        end
        Image_Stack_Complex_Aved=Temp/Ave_PostIFFT;
        Image_Stack_Phase=unwrap(angle(Image_Stack_Complex_Aved),[],3);
        %%
        Ave_PostIPhase=100;
        Temp=0;
        ...
        Phase_Image=max(Phase_Image(:))-Phase_Image;
        Phase_Image_norm=(Phase_Image-min(Phase_Image(:)))/(max(Phase_Image(:))-min(Phase_Image(:)));
        %
        imagesc(Phase_Image_norm);
        imwrite(Phase_Image_norm,[Data_Save_Folder sprintf('%s_PhaseMap.png',folder_list(QQQ).name)],'png');

        Phase_Image_Latnorm=Phase_Image;
        Phase_Image_Latnorm=Phase_Image./(repmat(mean(Phase_Image,2),[1 size(Phase_Image,2)]));
        Phase_Image_Latnorm_norm=(Phase_Image_Latnorm-min(Phase_Image_Latnorm(:)))/(max(Phase_Image_Latnorm(:))-min(Phase_Image_Latnorm(:)));
