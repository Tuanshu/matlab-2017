...
Save_File_Name='170620_Numerical Focusing Test';
root_folder_path='\';
last_folder_name=[1];
...
Axial_Ave_Factor=2*4; %8*8=64, *4 for 4-point
...
%% Filtering
    ...
    if If_Numerical_Focusing == 0
        Image_Stack_Hilbert=2*abs(ifft(FFT_Stack_Filtered,[],3));
    elseif If_Numerical_Focusing == 1
        Image_Stack_iFFT=ifft(FFT_Stack_Filtered,[],3);
        Image_Stack_iFFT_XFFT=fft(Image_Stack_iFFT,[],1);
        Image_Stack_iFFT_XFFT_Adj=Image_Stack_iFFT_XFFT.*exp()
    end
