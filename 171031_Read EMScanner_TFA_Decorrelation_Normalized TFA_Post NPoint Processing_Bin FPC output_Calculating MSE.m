...
last_folder_name='170719_Vaseline';
Number=[8];
...
%%
    ...
    %A=ifft2(abs(fft2(Temp_Image)).^2)
            %
        %NNN=5;
                %imagesc(squeeze(Image_Stack(50:100,p,250:330)))
        %imagesc(fftshift(fftshift(squeeze(abs(A))',1),2))

        %imagesc(squeeze(mean(fftshift(fftshift((C(:,:,:)),1),3),2))')
        %caxis([0 100000])
        %%
        end
    ...
    if If_STFT_1D
        ...
        set(gca,'YTick',(1:8:length(Wavelength_of_Interest_Array)),'YTickLabel',round(downsample(Wavelength_of_Interest_Array/Predicted_Central_Wavelength*N*ave_factor,8)),'FontSize', 14);
        xlabel('Temporal (Axial) Bins');
    ...
    if If_Save_Raw == 1
        ...
        fwrite(fid, Image_Stack(:,:,size(Image_Stack,3):-1:1), 'double');
        fclose(fid);
        fclose('all');
    end
end
