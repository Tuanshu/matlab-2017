...
for QQQ=1:length(folder_list)
   ...
   %% FFT to Frequency Domain
    ...
    if If_Speckle_Size_Full_Frame == 1
        ...
        imagesc(squeeze(mean(Auto(:,:,:),2))');

        imagesc(squeeze(-Speckle_Contrast_Stack(:,30,:))');
        caxis([40 50])
        %plot(squeeze(((Ratio(500,5,:))));

        plot(squeeze(mean(mean(Speckle_Contrast_Stack(:,:,:),2),1)));
        %% Save Raw of Speckle
        Processed_Data_Path_Speckle=[Data_Save_Folder sprintf('%s_Speckle_DR%g.raw',folder_list(QQQ).name,Downsampling_Ratio)];
