...
root_folder_path='D:\U\young\';
last_folder_name=[2 8 10 11 13 14];%
...
%Y_Ave_Factor=8;
Product_of_Axial_Decimation_Factor_and_Ave_Factor=round(16);
...
%%
    ...
    if If_Save_3D ==1
        if If_Enhace_Deep_Signal
            ...
            if length(last_folder_name)>1
                Processed_Data_Path=[Processed_Sub_Folder_Name sprintf('%03d_%s.raw',QQQ,System)];
            elseif Y_Offset~=0
                Processed_Data_Path=[Processed_Sub_Folder_Name sprintf('%03d_%s_Y%d.raw',last_folder_name(QQQ),System,Y_Offset)];
            else
                Processed_Data_Path=[Processed_Sub_Folder_Name sprintf('%03d_%s.raw',last_folder_name(QQQ),System)];
            end
        else
        ...
        fid = fopen(Processed_Data_Path, 'w+');
        fwrite(fid, flip(Reduced_Stack,3), 'double');
        fclose(fid);
    end
...
%%        
    ...
    if If_Enhace_Deep_Signal
        ...
        elseif Y_Offset~=0
            imwrite(Lateral_Binned_Image,[Processed_Sub_Folder_Name sprintf('%03d_%s_Y%d.png',last_folder_name(QQQ),System,Y_Offset)],'png');
        else
            imwrite(Lateral_Binned_Image,[Processed_Sub_Folder_Name sprintf('%03d_%s.png',last_folder_name(QQQ),System)],'png');
        end
    else
        Processed_Sub_Folder_Name=[Data_Save_Folder sprintf('System_%s_AAF_%d_LAF_%d_RBF_%d_CBF_%d_ABTH_%g_AVE_%d_Phase_%d_ProductAVE_%d_Colomn_%d',System,Axial_ave_Factor,Lateral_Ave_Factor,Row_Binning_Factor,Column_Binning_Factor,Auto_Brightness_TH,ave_factor,Phase_Shift,Product_of_Axial_Decimation_Factor_and_Ave_Factor,Colomn) '\'];
        if exist(Processed_Sub_Folder_Name)==0
