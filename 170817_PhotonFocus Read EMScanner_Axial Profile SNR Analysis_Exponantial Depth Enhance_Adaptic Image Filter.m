...
PreAVE=1;%Product_of_Axial_Decimation_Factor_and_Ave_Factor;%[4*4*4];
...
for QQQ=1:length(folder_list)
  ...
  %% Image Filter
    if If_Image_Filter == 1
        Start_Z_Index=Glass_Interface_Index;
        Start_sigX=0.001;
        End_SigX=3;
        sig_X=3;
        sig_Y=sig_X;
        Filter_2D_Half_Size=round(max(sig_X,sig_Y)*3);
        Filter_2D_Size=Filter_2D_Half_Size*2+1;
        X=1:Filter_2D_Size;
        Y=1:Filter_2D_Size;
        X_Gauss=gaussmf(X,[sig_X Filter_2D_Half_Size+1]);
        Y_Gauss=gaussmf(Y,[sig_Y Filter_2D_Half_Size+1]);
        Filter=repmat(X_Gauss,[Filter_2D_Size 1]).*repmat(Y_Gauss',[1 Filter_2D_Size]);
        Filter=Filter./sum(Filter(:));
        Reduced_Image=filter2(Filter,Reduced_Image,'same');
    end
  ...
  %%
    ...
    if If_Enhace_Deep_Signal
        Depth_Fit_Range=[120 390];  %micron
        ...
        Reduced_Image=Reduced_Image.*repmat(Exponential_Enhace_Coef',[1 size(Reduced_Image,2)]);
