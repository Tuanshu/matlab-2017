...
%%
cd('D:\MATLAB\TsLib');
root_folder_path='D:\171205_IG with 030-A\';
Given_sub_folder_path='Patial_030-A_22fps_28ms_SFR_B120_8to3V_R2.5_B1.1_1';

M=20;

System='IMX264with030ADefocus';
Color='M';  %R, G, B, M
X_ROI=[501 2000];
Y_ROI=[651 1650];
BayerBin=2;

ESF_Est_Type=2; % 1 and 2 for error function rought estimation, 3 for fermi, 4: (170906) FWHM
If_Best_Only=0;
Best_Index=65;
...
elseif strcmp(System,'EM2PL_ExtendFOV')
    ...
    Adaptor_M=200/180;
elseif strcmp(System,'EM3')
    Camera_Pixel_Size=10.6; %micron
    Adaptor_M=200/180;
    
elseif strcmp(System,'EM3Y')
    Camera_Pixel_Size=10.6; %micron
    Adaptor_M=200/180;
elseif strcmp(System,'IMX264with030ADefocus')
    Camera_Pixel_Size=3.45; %micron
    Adaptor_M=3.57806400004/20/BayerBin;
...
for FolderNumber=1:TotalFolderNumber
    if strcmp(Given_sub_folder_path,'')
        ...
        folder_path=[root_folder_path '\' sprintf('%s',sub_folder_path) '\'];
    ...
    for FileNumber=1:TotalFileNumber
        ...
        for FrameNumber=1:TotalFrameNumber
            ...
            elseif strcmp(System,'EM2PL_ExtendFOV')
                Data_Image_Raw=double(imread(file_path,'tiff',FrameNumber))';%.*FF;
                Data_Image_Raw=Data_Image_Raw(end:-1:1,:,:);
           elseif strcmp(System,'EM3')
                Data_Image_Raw=double(imread(file_path,'tiff',FrameNumber))';%.*FF;
                Data_Image_Raw=Data_Image_Raw(end:-1:1,:,:);
           elseif strcmp(System,'EM3Y')
                Data_Image_Raw=double(imread(file_path,'tiff',FrameNumber))';%.*FF;
                Data_Image_Raw=rot90(Data_Image_Raw,1);
            elseif strcmp(System,'OlympusDP73')
                Data_Image_Raw=double(imread(file_path,'tiff',FrameNumber))';%.*FF;
                Data_Image_Raw=Data_Image_Raw(end:-1:1,:,:);
            elseif strcmp(System,'IMX264with030ADefocus')
                %Data_Image_Raw=mean(double(imread(file_path,'tiff')),3);%.*FF;
                Data_Image_Input=imread(file_path,'tiff',FrameNumber);%.*FF;
                if strcmp(Color,'R')
                    Data_Image_Raw=double(Data_Image_Input(Y_ROI(1):Y_ROI(2),X_ROI(1):X_ROI(2),1));
                elseif strcmp(Color,'G')
                    Data_Image_Raw=double(Data_Image_Input(Y_ROI(1):Y_ROI(2),X_ROI(1):X_ROI(2),2));
                elseif strcmp(Color,'B')
                    Data_Image_Raw=double(Data_Image_Input(Y_ROI(1):Y_ROI(2),X_ROI(1):X_ROI(2),3));
                elseif strcmp(Color,'M')
                    Data_Image_Raw=double(mean(Data_Image_Input(Y_ROI(1):Y_ROI(2),X_ROI(1):X_ROI(2),:),3));
                end
                %Data_Image_Raw=Data_Image_Raw(:,end:-1:1);
                Data_Image_Raw=TSBinning(TSBinning(Data_Image_Raw,1,BayerBin),2,BayerBin);
            else
                %Data_Image_Raw=mean(double(imread(file_path,'tiff')),3);%.*FF;
                Data_Image_Raw=double(imread(file_path,'tiff',FrameNumber));%.*FF;
...
%                     end 
        ...
        %     xlim([min(New_Position_Array) max(New_Position_Array) ]);
            ...
            saveas(gcf,[root_folder_path '\' sub_processed_path '\' sub_folder_path sprintf('_Fig%d_%g',Acc_FrameNumber,min(Resolution_Array)) '_Butterfly.png'])
