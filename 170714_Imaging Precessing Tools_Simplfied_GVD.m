...
Time_Window=500;
nBin=1024;
Spectrogram_X_ROI=[200 800];%[200 800];
...
last_folder_name='170713_1024x65 test';
Number=[13];
...
%         plot(diff(squeeze(Image_Stack_Phase_Aved(XX,YY,:))));
    ...
    if If_STFT_GVD==1
        %%
        Image_Spectrogram_norm=(Image_Spectrogram-repmat(min(Image_Spectrogram,[],2),[1 size(Image_Spectrogram,2) 1]))./(repmat(max(Image_Spectrogram,[],2),[1 size(Image_Spectrogram,2) 1])-repmat(min(Image_Spectrogram,[],2),[1 size(Image_Spectrogram,2) 1]));
   %%
        NNN=611;
...
%     caxis([0 1]);
        ...
        if If_Color == 1
            ...
            elseif strcmp(Contrast,'PhaseMap_New')
                Color_Map_Ori=Phase_Image_Latnorm_norm_divbyNpoint;  
            end
