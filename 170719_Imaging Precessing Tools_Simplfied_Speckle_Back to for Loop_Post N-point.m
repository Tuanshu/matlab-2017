   ...
   %% FFT to Frequency Domain
    ...
    if If_Partial_Spectrum ==1
        ...
        if If_Hilbert == 0
            ...
            Phase_Stack=flip(Phase_Stack,3);
        else
    ...
    if If_Lateral_Phase_Analysis ==1
        %%
        plot(Phase_Stack(:,6,3000));
        xlim([450 550]);
        %%
        Order=1;
        Lateral_Phase_Step=pi;  %if The Phase
        ...
        %%
        Lateral_Phase_Step=pi*0.3;  %if The Phase
        Phase_Stack_XEdge=Phase_Stack_XDiff>Lateral_Phase_Step;
        Phase_Stack_XEdge(size(Phase_Stack_XEdge,1)+Order,:,:)=0;
        Phase_Stack_YEdge=Phase_Stack_YDiff>Lateral_Phase_Step;
        Phase_Stack_YEdge(:,size(Phase_Stack_YEdge,2)+Order,:)=0;
        Phase_Stack_Edge=Phase_Stack_XEdge;% | Phase_Stack_YEdge;
        %%
        Axial_Phase_Step=pi*1;  %if The Phase
        Phase_Stack_ZEdge=Phase_Stack_ZDiff>Axial_Phase_Step;
        Phase_Stack_ZEdge(:,:,size(Phase_Stack_ZEdge,3)+Order)=0;
        ...
        xlim([400 600]);
        ylim([3000 4000]);
        %% Add Axial Tor (因為peak to peak剛好為8 pixels, 故若令此線粗細變成4 pxiel, 則black to white ~= phase dsicontinouity -> p-1 to p+2, 4pixel
        Phase_Stack_ZEdge_Any=Phase_Stack_ZEdge;
        for p =1:size(Phase_Stack_ZEdge,3)
            Phase_Stack_ZEdge_Any(:,:,p)=any(Phase_Stack_ZEdge(:,:,max(1,p-1):min(p+2,size(Phase_Stack_ZEdge,3))),3);
            disp(p);
        end
     %%
        NNN=6;
        ...
        NNN=6;
        subplot(1,2,1)
        imagesc(squeeze(Phase_Watershed(:,NNN,:))');
        ...
        Order_2=2;
        Phase_Stack_ZEdge_Any_Xdiff=diff(Phase_Stack_ZEdge_Any,Order_2,1);
        Phase_Stack_ZEdge_Any_Ydiff=diff(Phase_Stack_ZEdge_Any,Order_2,2);
        Phase_Stack_ZEdge_Any_XEdge=Phase_Stack_ZEdge_Any_Xdiff ~= 0;
        Phase_Stack_ZEdge_Any_YEdge=Phase_Stack_ZEdge_Any_Ydiff ~= 0;
        Phase_Stack_ZEdge_Any_XEdge(size(Phase_Stack_ZEdge_Any_XEdge,1)+Order_2,:,:)=0;
        Phase_Stack_ZEdge_Any_YEdge(:,size(Phase_Stack_ZEdge_Any_YEdge,2)+Order_2,:)=0;
        Phase_Stack_ZEdge_Any_Edge=Phase_Stack_ZEdge_Any_XEdge | Phase_Stack_ZEdge_Any_YEdge;
