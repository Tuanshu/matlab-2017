...
%% Read BinFPC
Folder_Path='F:\AMO\171110_Backward txt-based Test\Processed Data\txt\';
...
plot(Time_New_NoNaN/1000,Voltage_Waveform_New_Equalspacing_noNaN,Time_New_NoNaN/1000,COEFS.a0+COEFS.a1*Time_New_NoNaN.^0.5+COEFS.a2*Time_New_NoNaN,'LineWidth',2);
xlabel('Time (sec)');
ylabel('Voltage (volt)');
legend('Arbitrary Voltage Waveform','3-parameter Fitted Voltage Waveform')
xlim([1 50])
...
%% Generating abitrary V wavbeform
Time_ms_for_output=min(Time_New_NoNaN):max(Time_New_NoNaN);
V_for_output=interp1(Time_New_NoNaN,Voltage_Waveform_New_Equalspacing_noNaN,Time_ms_for_output);
plot(Time_ms_for_output,V_for_output);
M=Multi;
V_for_output_M = decimate(V_for_output,M);
dlmwrite([Folder_Path 'Voltage Waveform.txt'],V_for_output_M','delimiter','\t','newline','pc','precision', '%.6f','-append');
