...
for Number=1:TotalNumber
    ...
    Pixel_Phase_Array=repmat(rem(0:Pixel_Phase_Per_Lateral_Pixel:Pixel_Phase_Per_Lateral_Pixel*(size(Data_Image_Raw,2)-1),1),[Number_of_Edge 1]);
...
%     end
    ...
    for p=1:Number_of_Edge
        for q=1:floor(size(Data_Image_Raw,2)/Down_Sample_Ratio)
                Edge_Array(:,(p-1)*floor(size(Data_Image_Raw,2)/Down_Sample_Ratio)+q)=Data_Image_Raw((round(Y_Index_Array(p,(q-1)*Down_Sample_Ratio+1))-Edge_Window_Half_Size):(round(Y_Index_Array(p,(q-1)*Down_Sample_Ratio+1))+Edge_Window_Half_Size),(q-1)*Down_Sample_Ratio+1);
        end
    end
