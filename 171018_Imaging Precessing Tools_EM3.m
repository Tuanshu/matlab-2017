clear all
%%
Task='AxialResolution';    %CCDZ or PZT or AxialResolution
%% Basic Operations
If_normalization=1;
If_DF=1;
If_FF=0;
...
elseif strcmp(Task,'PZT')
    ...
    If_AxialRes_Map=0;
elseif strcmp(Task,'AxialResolution')
    If_Partial_Spectrum=1;
    BW=0.05;        %Guassian sig
    Offset=0;    %percent
    If_Hilbert=1;   %Output the abs (env)
    If_AxialRes_Map=1;
    FPC=4*2;
    Axial_SR_Raw=0.78/2/FPC;    %micron, OPD (free space)
end
...
last_folder_name='171018_EM2 E-scan Glass Test';
Number=[3:4];
...
for QQQ=1:length(folder_list)
   ...
   %% FFT to Frequency Domain
    ...
    if If_Partial_Spectrum ==1
        ...
        %%
        plot(squeeze(Image_Stack(500,500,:)))
                %imagesc(squeeze(Image_Stack(:,6,:))')

    end
    %% Axial Res Map
    if If_AxialRes_Map == 1
        FWHM_Search_Range=100;
        [Max_Map Max_Index_Map]=max(Image_Stack,[],3);
        Image_Stack_NearGlass=Image_Stack(:,:,(floor(mean(Max_Index_Map(:)))-FWHM_Search_Range):(floor(mean(Max_Index_Map(:)))+FWHM_Search_Range));
        %%
        FWHM_Map=zeros(size(Image_Stack_NearGlass,1),size(Image_Stack_NearGlass,2));
        for p=1:size(Image_Stack_NearGlass,1)
            for q=1:size(Image_Stack_NearGlass,2)
                FWHM_Map(p,q)=Axial_SR_Raw*(find(Image_Stack_NearGlass(p,q,:)>Max_Map(p,q)/2,1,'last')-find(Image_Stack_NearGlass(p,q,:)>Max_Map(p,q)/2,1,'first'));
            end
        end
        imagesc(FWHM_Map);
        axis off
        axis equal
        title('Axial Resolution Map');
        caxis([1.8 2])
        colorbar

        fig = gcf;
        fig.PaperPositionMode = 'auto';
        saveas(gcf,[Data_Save_Folder sprintf('%s_AxialMap.png',folder_list(QQQ).name)]);
