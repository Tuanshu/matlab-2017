...
for Number=1:TotalNumber
    ...
    Window=25;
    Gauss_Filter=fspecial('gaussian',Window,Window/3);
    Blur_Image=filter2(Gauss_Filter,Data_Image_Raw,'valid');
    imagesc(Blur_Image)
    %%
    X_grad=[diff(Blur_Image,1,2) zeros(size(Blur_Image,1),1)];
    Y_grad=[diff(Blur_Image,1,1);  zeros(1,size(Blur_Image,2))];
    ...
    Angle_Map=atan(Y_grad./X_grad)*180/pi;
