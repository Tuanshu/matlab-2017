...
for Number=1:TotalNumber
    ...
    Grad_Top10perc=mean(Abs_grad_sort(1:round(length(Abs_grad_sort)/10)));
    
    Grad_Mask=Abs_grad>Grad_Top10perc;
    
    Blur=10;
    
    Blur_Filter=fspecial('gaussian',Blur,Blur/3);
    
    Grad_Mask_Blur=filter2(Blur_Filter,Grad_Mask,'valid');
    
    %Angle_Map=atan(X_grad./Y_grad)*180/pi;
    
    
    Grad_Grad_X=filter2(X_grad_Filter,Grad_Mask_Blur,'valid');
    Grad_Grad_Y=filter2(Y_grad_Filter,Grad_Mask_Blur,'valid');
    Angle_Map=atan(Grad_Grad_X./Grad_Grad_Y)*180/pi;
    imagesc(Angle_Map);
    
    imagesc(Grad_Mask.*Angle_Map);

    %%
    Masked_Weighted_Angle=Grad_Mask.*Abs_grad.*Angle_Map;
    Masked_Weighted_Angle=Masked_Weighted_Angle(:);
    Weighting=Grad_Mask.*Abs_grad;
    Weighting=Weighting(:);
    Weighting(isnan(Masked_Weighted_Angle))=[];
    Masked_Weighted_Angle(isnan(Masked_Weighted_Angle))=[];
    
    Weighted_Angle=sum(Masked_Weighted_Angle)/sum(Weighting);

     D_L_R=abs((1+Left_Bound)/2-(2*size(Data_Image_Raw,2)-Right_Bound+1)/2);
     LR_Extreme=L_Extreme-tan((Weighted_Angle)/180*pi)*D_L_R;
      [L_Extreme LR_Extreme R_Extreme]
